...
--insert into #NHMemberIDTemp (NHMemberID) values ('EH202213756736')
--insert into #NHMemberIDTemp (NHMemberID) values ('EH202213885022')

--insert into #NHMemberIDTemp (NHMemberID) values ('EH202213334561')   --- 146034

--insert into #NHMemberIDTemp (NHMemberID) values ('EH202214461722') --# 146023

insert into #NHMemberIDTemp (NHMemberID) values ('EH202214266916')



 -- To generate the SQL to execute in the NHCRM database to capture the Funds Disbursed
declare @NhlinkID varchar(20)
declare @InsCarrierID varchar(20)
declare @InsHealthPlanID varchar(20)
declare @PanProxyNumber varchar(20)
...
select @InsCarrierID = InsuranceCarrierID from master.MemberInsurances with (nolock)  where MemberID in (select memberID from master.members with (nolock)  where NHMemberID in (select NHmemberID from #NHMemberIDTemp))
select @InsHealthPlanID = InsuranceHealthPlanID from master.MemberInsurances with (nolock)  where MemberID in (select memberID from master.members with (nolock)  where NHMemberID in (select NHmemberID from #NHMemberIDTemp))
select @PanProxyNumber = CardReferenceNumber from member.membercards where NHMemberID in (select NHmemberid from #NHmemberIDTemp )

select  @NhlinkID as NHLinkID, @InsCarrierID as InsCarrierID, @InsHealthPlanID as InsHealthPlanID, @PanProxyNumber as PanProxyNumber


-- to generate the SQL to execute in NHCRM

-- This script is to generate the scripts to execute in NHCRM for all Benefits and Funds Transferred
select 1 as OrderNo,
...
,* from elig.[EHPicksEligBenefitData] with (nolock) where NHLinkId = ' + ''''+ @NHLinkID+'''' + ' Order by FileTrackID Desc ' as FundsDisbursedScriptInNHCRM

union
select 2 OrderNo,
'select distinct '+''''+'flex.FISWalletMapping'+''''+ ' as TableName, ' +''''+'FISWalletMapping' +''''+ ' as FISWalletMapping_Query, IsActive, FIS_PurseName, FIS_PurseSlot, NB_WalletCode, CarrierID, HealthPlanID from flex.FISWalletMapping with (nolock)  where CarrierID =  ' + @InsCarrierID+ ' and HealthPlanID = '+ @InsHealthPlanID as FundsDisbursedScriptToExecuteInNHCRM

union
select 3 OrderNo,
'select '+''''+'otcfunds.CardBenefitLoad_CI'+''''+ ' as TableName, '+ ''''+'CardInitiated Information to FIS' +''''+ 'as CardInitiatedInformationToFIS_Query, IsActive, NBWalletCode, BenefitAmount, BenefitValidFrom, BenefitValidTo, * from otcfunds.CardBenefitLoad_CI with (nolock) where NHLinkid = ' + ''''+ @NHLinkID+'''' as FundsDisbursedScriptToExecuteInNHCRM


union
select 4 OrderNo,
'select '+''''+'otcfunds.CardBenefitLoad_FD'+''''+' as TableName, '+''''+'FundsDisbursed Information to FIS'+''''+ ' as FundsDisbursedInformationToFIS_Query, memberdatasourcefile, recordtype, IsActive, NBWalletCode, BenefitAmount, BenefitValidFrom, BenefitValidTo, * from otcfunds.CardBenefitLoad_FD with (nolock) where NHLinkid = '+ ''''+ @NHLinkID+'''' as FundsDisbursedScriptToExecuteInNHCRM

union
-- Information from FIS
select 5 OrderNo,
'select ' +''''+'fisxtract.Monetary' +''''+ ' as TableName, '+''''+'Monetary Information from FIS' +''''+ 'as MonetaryInformationFromFIS_Query, CreateDate, PANProxyNumber,CardNumberProxy, ClientName, SettleAmount, TxnLocalAmount, txnSign, MerchantName, PurseName, PurseNo, PurseStatus,PurseCreationDate, PurseEffectiveDate, PurseStatusDate,  ResponseDescription, * from fisxtract.Monetary with (nolock)  where PANProxyNumber = ' + @PanProxyNumber + ' Order by 3 Desc ' as FundsDisbursedScriptToExecuteInNHCRM

union
select 6 OrderNo,
'select '+''''+'fisxtract.Monetary' +''''+ ' as TableName,' + ''''+'Monetary Information from FIS Balance' +''''+  'as MonetaryInformationFromFISBalance_Query, PANProxyNumber,CardNumberProxy,PurseName, sum(TxnLocalAmount*TxnSign) as Balance from fisxtract.Monetary with (nolock)  where PANProxyNumber = ' + @PanProxyNumber + ' group by PANProxyNumber,CardNumberProxy, PurseName' +  ' Order by 3 Desc '  as FundsDisbursedScriptToExecuteInNHCRM

union
select 7 OrderNo,
'select '+''''+'fisxtract.NonMonetary' +''''+ 'as TableName,'+ ''''+'NonMonetary Information from FIS' +''''+  'as NonMonetaryInformationFromFIS_Query, ' + 'CreateDate, PANProxyNumber,CardNumberProxy, ClientName, ProgramName, CardStatus, ActivateDate,CardNumberLocStatusEffectiveDate,  CardStatusDescription, RequestCodeDescription, SourceCodeDescription, Memo_Comments_ClientDefined,   * from fisxtract.NonMonetary with (nolock)  where PANProxyNumber = ' +@PanProxyNumber + ' Order by 3 desc ' as FundsDisbursedScriptToExecuteInNHCRM

union

select 8 OrderNo,
'select ' +''''+'fisxtract.[Authorization]'+''''+ ' as TableName,' + + ''''+'Authorization Information from FIS' +''''+  'as AuthorizationInformationFromFIS_Query, ' + 'CreateDate, PANProxyNumber,CardNumberProxy, IssuerClientName, SubProgramName, TxnTypeName, PurseName, TransactionDate_Time, ActualRequestCodeDescription, txnLocalAmount, TransactionSign, MerchantName, AuthorizationBalance, SettleBalance, ReasonIDDescription, SourceDescription, * from fisxtract.[Authorization] with (nolock)  where PANProxyNumber =  ' +@PanProxyNumber + ' Order by 3 desc '  as FundsDisbursedScriptToExecuteInNHCRM
...
,* from elig.[EHPicksEligBenefitData] with (nolock) where NHLinkId in (Select NHLinkID from master.Members where NHMemberID in (select NHMemberID from #NHmemberIDTemp))
...
select distinct 'flex.FISWalletMapping' as TableName, 'FISWalletMapping' as FISWalletMapping_Query, IsActive, FIS_PurseName, FIS_PurseSlot, NB_WalletCode, CarrierID, HealthPlanID from flex.FISWalletMapping with (nolock)  where
CarrierID in  (select distinct InsuranceCarrierID from master.MemberInsurances with (nolock)  where MemberID in (select memberID from master.members with (nolock)  where NHMemberID in (select NHmemberID from #NHMemberIDTemp)))
and HealthPlanID in (select distinct InsuranceHealthPlanID from master.MemberInsurances with (nolock)  where MemberID in (select memberID from master.members with (nolock)  where NHMemberID in (select NHmemberID from #NHMemberIDTemp)))
...
( 
		...
		from insurance.ContractRules a with (nolock) 
		left join Insurance.HealthPlanContracts b with (nolock)  on a.HealthPlanContractID = b.HealthPlanContractID
		left join rulesengine.BenefitRulesData c with (nolock)  on  a.BenefitRuleDataID  = c.BenefitRuleDataID
		left join insurance.InsuranceCarriers ins with (nolock)  on b.InsuranceCarrierID = ins.InsuranceCarrierID
		left join insurance.InsuranceHealthPlans ihp with (nolock) on b.InsuranceHealthPlanID = ihp.InsuranceHealthPlanID
		where 1=1 and a.isActive = 1 and b.IsActive = 1 and c.IsActive =1 and ins.IsActive =1 and ihp.IsActive = 1
		and ins.InsuranceCarrierID in (select distinct InsuranceCarrierID from master.MemberInsurances with (nolock)  where MemberID in (select memberID from master.members where NHMemberID in (select NHmemberID from #NHMemberIDTemp)))
		and (ihp.InsuranceHealthPlanID in (select distinct InsuranceHealthPlanID from master.MemberInsurances with (nolock)  where MemberID in (select memberID from master.members where NHMemberID in (select NHmemberID from #NHMemberIDTemp)))
			 --or ihp.InsuranceHealthPlanID is null
			 )
)
and InsCarrierID in (select distinct InsuranceCarrierID from master.MemberInsurances with (nolock)  where MemberID in (select memberID from master.members where NHMemberID in (select NHmemberID from #NHMemberIDTemp)))
--and InsHealthPlanID = 4197
and InsHealthPlanID in (select distinct InsuranceHealthPlanID from master.MemberInsurances with (nolock)  where MemberID in (select memberID from master.members where NHMemberID in (select NHmemberID from #NHMemberIDTemp)))
*/
...
select 'insurance.ContractRules' as insurance_ContractRules_TableName, 

ContractRuleID, IsActive, CreateDate, EffectiveFrom, EffectiveTo, BenefitRuleDataID, HealthPlanContractID, PublishVersionID
...
select 'rulesengine.BenefitRulesData' as rulesengine_BenefitRulesData_TableName, IsActive,BenefitRuleData,
...
,BenefitRuleDataID, BenefitRuleID, CreateDate
...
-- to check catalogs
select distinct '[SupplOrders].[PlanItemsConfig]' as TableName_SupplOrders_PlanItemsConfig,[PlanItemsConfigID],[ItemMasterID],[InsuranceCarrierID],[InsuranceHealthPlanID],[CreateUser],[CreateDate],[ModifyUser],[ModifyDate],[IsActive],[Price] from [SupplOrders].[PlanItemsConfig]
where InsuranceCarrierID in (select distinct InsuranceCarrierID from master.MemberInsurances where MemberID in (select memberID from master.members where NHMemberID in (select NHmemberID from #NHMemberIDTemp)))
and InsuranceHealthPlanID in (select distinct InsuranceHealthPlanID from master.MemberInsurances where MemberID in (select memberID from master.members where NHMemberID in (select NHmemberID from #NHMemberIDTemp)))
