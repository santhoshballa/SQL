...
select * from elig.mstrEligBenefitData where FirstName = 'DELORES' and LastName = 'ROSE' and IsActive =1 and insCarrierID = 278 and insHealthPlanID = 3438 
select NHMemberID from master.members where NHLInkid = '90006649701'
90006649701	NH202313540890
select  * from elig.mstrEligBenefitData where insCarrierID = 278 and insHealthPlanID = 3438 and IsActive =1 and GroupNbr = 'H2563' and BenefitEndDate > '07-18-2023'

select 
'insert into #NHMemberIDTemp (NHMemberID) values ('+''''+ NHMemberID + ''''+ ')' from master.members where NHLInkid in (select  NHLinkID from elig.mstrEligBenefitData where insCarrierID = 278 and insHealthPlanID = 3438 and IsActive =1 and GroupNbr = 'H2563' and BenefitEndDate > '07-18-2023')
...
select distinct NHMemberID, NHLinkID, * from master.members where NHLInkID in ('90005190101', '90006606601')

insert into #NHMemberIDTemp (NHMemberID) values ('NH202002403248')
insert into #NHMemberIDTemp (NHMemberID) values ('NH202106521128')
insert into #NHMemberIDTemp (NHMemberID) values ('NH202107384149')
insert into #NHMemberIDTemp (NHMemberID) values ('NH202315071852')


select top 100 * from information_schema.routines where routine_name like '%cardissue%'
...
insCarrierID	insHealthPlanId	ProductID
4	5	MA006

select distinct NHLinkID from elig.mstrEligBenefitData where InsCarrierID = 4 and InsHealthPlanID = 5 and IsActive = 1 and BenefitEndDate > '07-20-2023' and ProductID  = 'MA006' and NHLinkID = '648000198401'

select distinct NHMemberID from master.Members where NHLinkID in (select distinct NHLinkID from elig.mstrEligBenefitData where InsCarrierID = 4 and InsHealthPlanID = 5 and IsActive = 1 and BenefitEndDate > '07-20-2023' and ProductID  = 'MA006')
...
CarrierID in  (select distinct InsuranceCarrierID from master.MemberInsurances with (nolock)  where MemberID in (select memberID from master.members with (nolock)  
where NHMemberID in (select distinct NHMemberID from master.Members where NHLinkID in (select distinct NHLinkID from elig.mstrEligBenefitData where InsCarrierID = 4 and InsHealthPlanID = 5 and IsActive = 1 and BenefitEndDate > '07-20-2023' and ProductID  = 'MA006'))))
and HealthPlanID in (select distinct InsuranceHealthPlanID from master.MemberInsurances with (nolock)  
where MemberID in (select memberID from master.members with (nolock)  where NHMemberID in (select distinct NHMemberID from master.Members where NHLinkID in (select distinct NHLinkID from elig.mstrEligBenefitData where InsCarrierID = 4 and InsHealthPlanID = 5 and IsActive = 1 and BenefitEndDate > '07-20-2023' and ProductID  = 'MA006'))))
and PurseFundCondition = 'GROCERYSSBCI'


select distinct 'flex.FISWalletMapping' as TableName, 'FISWalletMapping' as FISWalletMapping_Query, IsActive, BenefitCategoryValue, BenefitFrequencyMonths, PurseFundCondition, PurseStartDate, PurseEndDate,  from flex.FISWalletMapping with (nolock)  
where CarrierID in  (select distinct InsuranceCarrierID from master.MemberInsurances with (nolock)  where MemberID in (select memberID from master.members with (nolock)  where NHMemberID in (select distinct NHMemberID from master.Members where NHLinkID in (select distinct NHLinkID from elig.mstrEligBenefitData where InsCarrierID = 4 and InsHealthPlanID = 5 and IsActive = 1 and BenefitEndDate > '07-20-2023' and ProductID  = 'MA006'))))
and HealthPlanID in (select distinct InsuranceHealthPlanID from master.MemberInsurances with (nolock)  where MemberID in (select memberID from master.members with (nolock)  where NHMemberID in (select distinct NHMemberID from master.Members where NHLinkID in (select distinct NHLinkID from elig.mstrEligBenefitData where InsCarrierID = 4 and InsHealthPlanID = 5 and IsActive = 1 and BenefitEndDate > '07-20-2023' and ProductID  = 'MA006'))))
and PurseFundCondition = 'GROCERYSSBCI'

NB_WalletCode | CH02FIS23

select distinct 'flex.FISWalletMapping' as TableName, 'FISWalletMapping' as FISWalletMapping_Query, IsActive, BenefitCategoryValue, BenefitFrequencyMonths, PurseFundCondition, PurseStartDate, PurseEndDate  from flex.FISWalletMapping with (nolock) 
where CarrierID = 4 and HealthPlanID = 5


select distinct 'flex.FISWalletMapping' as TableName, 'FISWalletMapping' as FISWalletMapping_Query, IsActive, FIS_PurseName, FIS_PurseSlot, NB_WalletCode, CarrierID, HealthPlanID, BenefitCategoryValue, BenefitFrequencyMonths, PurseFundCOndition, CardIssuer, Level1ClientID, PurseStartDate, PurseEndDate from flex.FISWalletMapping with (nolock)  where
CarrierID in  (select distinct InsuranceCarrierID from master.MemberInsurances with (nolock)  where MemberID in (select memberID from master.members with (nolock)  where NHMemberID in ('NH202315296101')))
and HealthPlanID in (select distinct InsuranceHealthPlanID from master.MemberInsurances with (nolock)  where MemberID in (select memberID from master.members with (nolock)  where NHMemberID in ('NH202315296101')))
...
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202107249378') -- seq 202 | CFDP022123
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202313540890')  --   NH202313540890 203
  --insert into #NHMemberIDTemp (NHMemberID) values ('NH202315296101') -- NH202315296101, seq 186
-- insert into #NHMemberIDTemp (NHMemberID) values ('NH202313439427')  -- seq 186 -- similar example
  
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315321601')   --seq 203

--insert into #NHMemberIDTemp (NHMemberID) values ('NH202002403248')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202106521128')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202107384149')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315071852')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315108087')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315226299')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315273482')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315273503')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315301154')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315308086')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315326707')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315326709')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315594179')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315594160')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315594259')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315594248')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315594218')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315594246')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315594180')
--insert into #NHMemberIDTemp (NHMemberID) values ('NH202315594177')
...
-- for Optima Members only, An answer is required on the questionnaire for a card to be issued
select ma.*,q.SubCategory,q.Question,a.Answer
,CASE WHEN q.Question = 'Other' then ISNULL(ma.OtherAnswer,q.SubCategory) else q.Question end QuestionswithYES
from member.MemberAnswers ma 
join member.Questions q on q.QuestionId = ma.QuestionId and q.Category = 'OTC_HCQ'
join member.Answers a on a.AnswerId = ma.AnswerId where 1=1 and NHmemberID in (select NHmemberID from #NHMemberIDTemp)

-- This table contains the members that have Optin to Benefits, a record has to be inserted into this table for a card to be issued. 
/*
benefitcard.ChangeRequest.RequestData contains a JSON key value pair for UI configuration change for example a 90$ benefit is configured like this {"walletCode":"CFDP022123","amount":90.00}, check this
*/
...
-- Card is issued or not to a Carrier and a Plan
