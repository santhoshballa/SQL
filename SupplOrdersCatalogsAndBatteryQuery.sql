...
--'[supplOrders].[PlanItemsConfig]' as TableName, j.* 
from 
[supplOrders].[OrderItems] a 
left join [supplOrders].[Orders] b on (a.orderid = b.orderid)
left join [supplOrders].[ItemMaster] c on a.ItemCode = c.ItemCode
...
left join [Insurance].[InsuranceCarriers] h on h.InsuranceCarrierID = b.InsuranceCarrierID
left join [Insurance].[InsuranceHealthPlans] i on i.InsuranceHealthPlanID =  b.InsuranceHealthPlanID
--left join [supplOrders].[PlanItemsConfig] j on j.ItemMasterID = c.ItemMasterId

where
1=1 
and b.NHMemberID = 'NH201700008311'
--and a.orderid = 6041
...
-- No of Items in Each Order using windowing function Rank() and Partition by
select top 10
o.orderItemid, 
o.orderid, 
RANK() OVER (PARTITION BY o.OrderId ORDER BY o.OrderItemId) OrderItemRank
from [supplOrders].[OrderItems] o where o.orderid = 6041

/*
orderItemid	orderid	OrderItemRank
8116	6041	1
8117	6041	2
8118	6041	3
8119	6041	4
...
--NHMemberID | NH201700008311

select InsuranceCarrierID,InsuranceCarrierName, IsActive from insurance.InsuranceCarriers where InsuranceCarrierName like '%Ultimate Health Plan%'
/*
InsuranceCarrierID	InsuranceCarrierName	IsActive
14	Ultimate Health Plan	1
190	Ultimate Health Plan, Inc.	0
*/
...
select * from [Insurance].[InsuranceCarriers] where insuranceCarrierID in ( 258)
select * from [supplOrders].[ItemMaster] where ItemMasterID in (3,15, 71, 111, 112 )
...
where 1=1
--a.ItemMasterID = 
and a.InsuranceCarrierID in ( 258) and a.IsActive = 1

/*
Ticket # 46626
Remove Incomm catalogs
*/

Update [supplOrders].[PlanItemsConfig] set IsActive = 0 where ItemMasterID = 3 and InsuranceCarrierID = 258
Update [supplOrders].[PlanItemsConfig] set IsActive = 0 where ItemMasterID = 15 and InsuranceCarrierID = 258
*/